...
public class SpecificLedgerFinder extends Finder
{
    ...
    /**
     * Construct a blank finder. You'll have to set the account title and
     * ledger name manually. Useful for cases where you do several finds at
     * once.
     * 
     */
     ...
     *            title you are seeking. If it is blank, the Ledger name must
     *            be specific enough to result in the return of a single
     *            Ledger when {@link #getLedger()} is run.
     */
     ...
     * 
     * @param name
     *            not null or blank, but can be either all or part of the
     *            Ledger name you are seeking. If it is blank, the Account
     *            title must be specific enough to result in the return of a
     *            single Ledger when {@link #getLedger()} is run.
     */
    ...
    public List query(DataClient store) throws NotFoundException {
        ...
        this.result = store.nativeQuery(new Selector<Ledger>() {
            ...
            public boolean match(Ledger l) {
                try {
                    if (l.getName() == null) {
                        // a bit unusual to have null for a Ledger name, but
                        // it
     ...
     * 
     * @return the single Ledger that matches the title and name supplied to
     *         the constructor or setters.
     * @throws NotFoundException
     *             if no Ledger matched the search critera.
     * @throws UnsupportedOperationException
     *             if your search results in more than a single Ledger (ie,
     *             you have to narrow the search with more precise terms)
     */
    public Ledger getLedger() throws NotFoundException {
        ...
        if (!(obj instanceof Ledger)) {
            throw new IllegalStateException(
                    "In querying Ledgers, you managed to get something not a Ledger!");
        }

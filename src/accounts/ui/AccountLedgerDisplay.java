...
/**
 * Display the title and name of an Account / Ledger pair. This widget wraps and
 * delegates to a Label, whose String is a complex series of Pango markup used
 * to achieve the correct colours and spacing.
 * <p>
 * AccountLedgerDisplay sets a minimum width that is very small. It is up to the
 * code using this widget to call
...
public class AccountLedgerDisplay extends HBox
{
	...
	/**
	 * Set the Account and Ledger shown. The Account (implicitly from the Ledger
	 * by following its parentAccount reference) and Ledger will be have their
	 * title/name marked up and used as the text of the Label underlying this
	 * Display widget.
	 * 
	 * @param ledger
	...
	public void setLedger(Ledger ledger) {
		 ...
		 * because otherwise when the total width gets divided into 2, half for
		 * each side, it's not wide enough for the longer label. This
		 * overshoots, but that is ok because it is used as an upper bound to
		 * decrement down from when trying to find a working width that does fit
	...
	/**
	 * Evaluate the space allocated to the Box that is this Widget and the size
	 * requested by the underlying Label in order to display the markup. Use
	 * this information to adjust the width variable as necessary and to
	 ...
	 * the appropriate colours with the appropriate separator character. This is
	 * a class method which can be used by other Widgets. and Ledger will be
	 ...
	 * @param width
	 *            the maximum width, in characters. This isn't precise, per se,
	 *            but is internally consistent as the number perterbs up or
	 *            down.
	 */

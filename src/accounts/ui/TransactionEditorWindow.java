 ...
 * we do it here in ok(), which subclasses are expected to call after they have
 * populated the transaction they are operating on.
 * <p>
 * The constructors pass straight through to EditorWindow. If a database id is
 * specified, the object is fetched and the TransactionEditorWindow marks itself
 * as editing.
 * 
 * @author Andrew Cowie
 */
public abstract class TransactionEditorWindow extends EditorWindow
{
	...
	/**
	 * A reflexive reference. We don't make this available externally, following
	 * a convention that if you want <code>self</code>, define it yourself in
	 * the class you need so it has the method visibility you'd expect of this.
	 */
	...
	private void setup(long id) {
		...
		if (id == 0) {
			...
			editing = false;
			/*
			 * You will have to populate transaction with the new object you are
			 * creating, and set a useful title.
			 */
		} else {
...
//			}
	...
	/**
	 * Execute the transaction to store or update the Transaction, doing so in a
	 * worker thread. Carry out any necessary guards before calling this.
	 */
	protected void ok() {
		/*

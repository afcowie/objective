...
/**
 * A Table container specialized to simply do two columns of aligned rows.
 * GTK's immensely powerful Table layout widget has terrific fine grain
 * placement control, but this unsurprisingly makes it a bit ugly to use for
 * simple cases. This class is a Table which simply has a left side and a
 * right side and aligns rows to be equally spaced. [Two VBoxes in an HBox
 * wouldn't achieve the same result because the rows on each side would not be
 * parallel; a series of HBoxes in a VBox would lack a common column
 * boundary].
 * 
 * @author Andrew Cowie
 */
public class TwoColumnTable extends Table
{
    ...
    public void attach(Widget widget, Align side) {
        ...
        /*
         * If there's an incomplete row, and we get a CENTER, we need a new
         * row:
         */
        ...
        if (rowCount > rowCapacity) {
            /*
             * Admittedly this is inefficient, expanding by unit size each
             * time this code is hit, but this can be avoided by specifying an

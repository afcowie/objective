...
public class VerboseResultPrinter extends TextOutput implements TestListener
{
	...
	public VerboseResultPrinter(boolean haltOnBug) {
		/*
		 * Calling super.toOutput(PrintStream) will wrap the PrintStream with a
		 * PrintWriter and then in turn call this.toOutput(PrintWriter).
		 * Convoluted perhaps, but might as well as it uses the abstract method
		 * we have to implement anyway.
		 */
	...
	/**
	 * Implements abstract toOutput() from TextOutput. We don't use this as the
	 * entry point in the way that the real TextOutput subclasses do; this just
	 * sets stdout to the output stream.
	 */
	...
	public void startTest(Test test) {
		...
		/*
		 * If we're in a new class, then switch our reference and print the new
		 * name.
		 */
		 ...
		 * "testCaseName(java.package.Class)". So we use a regular expression ot
		 * nuke out the characters between the brackers. We then use
		 * TextOutput's static routines to trim the output to terminal width
		 * less 10 but pad it out to that width. We then let the other callbacks
		 * finish the line.
		 */

...
/**
 * An extension of the standard JUnit textui TestRunner which displays the name
 * of the test being run. This class overrides the more obnoxious output methods
 * in TestRunner, and uses our VerboseResultPrinter to output the name of the
 * test being run instead of {.,F,E}
 * 
 * @author Andrew Cowie
 */
/*
...
{
	/**
	 * Create an instance of our VerboseTestRunner wrapper. It uses the default
	 * output stream (ie, System.out) but overrides JUnit's ResultPrinter with
	 * our customized VerboseResultPrinter.
	 */
	...
	/**
	 * VerboseTestRunner's main entry point. This call's TestRunner's getTest()
	 * method to parse the command line argument to find a Test{,Suite}, and
	 * then calls our doRun() to get a TestResult from executing that Suite.
	 * 
	 * @param args
	 *            VerboseTestRunner takes one (and only one) argument, the name
	 *            of the Test{,Suite} to run. From {@link TestRunner}: expects
	 *            the name of a TestCase class as argument. If this class
	 *            defines a static <code>suite</code> method it will be
	...
	/**
	 * Runs a single Test{,Suite} and collects its results. If you have a static
	 * Test suite() this method can be used to run it directly. For example,
	 * from a main() specific to a given class [hierarchy] you can do:
	 * 
	 * <pre>
	 * public static void main(String[] args) {
